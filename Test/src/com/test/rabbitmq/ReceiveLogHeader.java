...
public class ReceiveLogHeader {
    ...
    public static void main(String[] argv) throws Exception {
        if (argv.length < 1) {
            System.err
                .println("Usage: ReceiveLogsHeader queueName [headers]...");
            System.exit(1);
        }
        ...
        for (int i = 1; i < argv.length; i++) {
            ...
            System.out.println("Binding header " + argv[i] + " and value "
                + argv[i + 1] + " to queue " + queueInputName);
            i++;
        }

        String queueName = channel
            .queueDeclare(queueInputName, true, false, false, null).getQueue();
        channel.queueBind(queueName, EXCHANGE_NAME, routingKeyFromUser,
            headers);
